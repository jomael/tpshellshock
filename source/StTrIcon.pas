...
{*********************************************************}
{* ShellShock: StTrIcon.pas 1.02                         *}
{*********************************************************}
{* ShellShock: Component to put app in system tray       *}
{*********************************************************}
...
begin
  ...
  if not (csDesigning in ComponentState) then begin
    ...
    MI.Caption := ShellShockStr(ssscTrayIconClose);
    MI.OnClick := DoCloseClick;
    DefPopup.Items.Add(MI);

    MI := TMenuItem.Create(Self);
    MI.Caption := ShellShockStr(ssscTrayIconRestore);
...
begin
  ...
  if not NotifyIcon(NIM_ADD) then begin
    IsActive := False;
    RaiseStError(ESsTrayIconError, ssscTrayIconCantAdd)
  end else
    IsActive := True;
end;
...
procedure TStCustomTrayIcon.MinimizeApplication;
begin
...
begin
  ...
  if not (csDesigning in ComponentState) and IsActive then begin
    if not Assigned(Images) then
      RaiseStError(ESsTrayIconError, ssscImageListInvalid);
